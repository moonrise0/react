...
app.get("/assignment3", (request, response) => {
  fs.readFile("./html/assignment3.html", "utf-8", (err, data) => {
...
app.get("/assignment3", (request, response) => {
  let name = parseInt(request.query.name); //input태그의 name속성 ,userid는 서버로 연결하는데 도움이 안됨.
  ...
  response.send(`${kor} + ${eng} + ${math}= ${sum}`);
});
